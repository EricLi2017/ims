...
<%
	...
	String supplyId = Filter.nullFilter(request.getParameter("supplyId"));//hidden param
	...
	if (supplyId.isEmpty()) {
		out.print("Missing required parameter: Supply ID");
		return;
	}
	...
	int rows = new ProductSupplyDatabase().updateById(supply);

	//response
	String title = rows > 0 ? "Success" : "Failed";
	String titleColor = rows > 0 ? "green" : "red";
	String detail = "Update supply " + supplyId + " of product " + productId + ", " + rows + "  rows affected!";
