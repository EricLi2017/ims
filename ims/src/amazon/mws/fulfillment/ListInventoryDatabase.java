...
public class ListInventoryDatabase {
    /**
     * update :fnsku=?,total_supply_quantity=?,in_stock_supply_quantity=? by sku
     */
    public int update(List<InventorySupply> supplys) {
        int rows = 0;
...
//            System.out.println("sku condition fusku TotalSupplyQuantity InStockSupplyQuantity");//TODO for debug
...
//                System.out.println("");
                // Not in FBA inventory: the supply condition and fnsku will return null (and the supply inventory will return 0)
                if (supply.getCondition() == null || "NewItem".equalsIgnoreCase(supply.getCondition())) {
                    ps = con.prepareStatement(sql);
                    ps.setString(1, supply.getFNSKU());
                    ps.setString(2, (supply.getTotalSupplyQuantity() == null || (supply.getCondition() == null && supply.getTotalSupplyQuantity() == 0)) ? null : supply.getTotalSupplyQuantity().toString());
                    ps.setString(3, (supply.getInStockSupplyQuantity() == null || (supply.getCondition() == null && supply.getInStockSupplyQuantity() == 0)) ? null : supply.getInStockSupplyQuantity().toString());
