...
public class ListInventoryManager extends Observable {
     ...
     *
     * @param skus skus.size()<=SKU_MAX_NUM
     * @return updated rows
     */
    ...
    public int updateAll() {
        ...
        for (Product product : products) {
            //add to sku list,
            // and if sku size reach SKU_MAX_NUM should call MWS and reset the sku list
            skus.add(product.getSku());
            skuNum++;
            if (skuNum % SKU_MAX_NUM == 0 || skuNum == products.size()) {
                // call MWS and update inventory in database
                batchUpdateRows = updateInventory(skus);
                totalUpdateRows += batchUpdateRows;
                //set message
                msg = batchUpdateRows + "/" + skus.size() + " " + totalUpdateRows + "/" + products.size() + " SKU updated";
                System.out.println(msg);
                //notify observers to add a message
                addMessage(msg);
