...
public class ListOrdersManager {
	 ...
	 * @throws Exception
	 */
	public static int insertShippedOrders(XMLGregorianCalendar createdAfter, XMLGregorianCalendar createdBefore) throws SQLException
			 {
	...
	private static int insertDatabase(List<Order> orders) throws SQLException  {
	...
	public static int getCountFromDB(XMLGregorianCalendar createdAfter, XMLGregorianCalendar createdBefore) {
		if (createdAfter == null || createdBefore == null)
			return -1;
	...
	public static int getCountFromMWS(XMLGregorianCalendar createdAfter, XMLGregorianCalendar createdBefore) {
		if (createdAfter == null || createdBefore == null)
			return -1;
		int rows = 0;
		// Make the call.
	 ...
	 */
	public OrderSumMWS getSumFromMWS(XMLGregorianCalendar createdAfter, XMLGregorianCalendar createdBefore) {
		if (createdAfter == null || createdBefore == null)
			return null;
	...
	public class OrderSumMWS {
		...
		public String toString() {
			return "OrderSumMWS{" + "orderNum=" + orderNum + ", itemNum=" + itemNum + '}';
		}
	}
	...
	public static void main(String... strings) {
		...
		// .selectCountWithOrderItemsByPruchaseDate(Time.getTimeInPST(createdAfter),
		// Time.getTimeInPST(createdBefore));
		// int noRows = new ListOrdersAndOrderItemsDatabase()
		// .selectCountWithoutOrderItemsByPruchaseDate(Time.getTimeInPST(createdAfter),
		// Time.getTimeInPST(createdBefore));
		//
		// List<String> amazonOrderIds = new ListOrdersAndOrderItemsDatabase()
		// .selectOrderIdsWithoutOrderItemsByPruchaseDate(Time.getTimeInPST(createdAfter),
		// Time.getTimeInPST(createdBefore));
		//
		// System.out.println("there are " + rows + " rows with order items");
		// System.out.println("there are " + noRows + " rows without order items");
		// System.out.println("there are " + amazonOrderIds.size() + " rows without
		// order items");
	}
